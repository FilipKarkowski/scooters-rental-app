...
@section('content')
            ...
            <tr class="  hover:bg-zinc-600 hover:bg-opacity-10 transition-colors duration-300 ease-in-out">
                <td class="sm:p-3 py-2 px-1 border-b border-gray-200">
                    <div class="flex items-center">
                        {{$raport->updated_at->format('Y-m-d')}}
                    </div>
                </td>
                <td class="sm:p-3 py-2 px-1 border-b border-gray-200">
                    <div class="flex items-center">

                        {{ $raport->liczba_wypozyczen  }}
                    </div>
                </td>
                <td class="sm:p-3 py-2 px-1 border-b border-gray-200">
                    <div class="flex items-center">
                        {{ $raport->liczba_odbiorow }}
                    </div>
                </td>
                <td class="sm:p-3 py-2 px-1 border-b border-gray-200">
                    <div class="flex items-center">
                        {{ $raport->liczba_uszkodzonych }}
                    </div>
                </td>
                <td class="sm:p-3 py-2 px-1 border-b border-gray-200">
                    <div class="flex items-center">
                ...
                <td class="sm:p-3 py-2 px-1 border-b border-gray-200">
                    <div class="flex items-center">
                        {{ $raport->placowka->nazwa }}
                    </div>
                </td>

                @if(Auth::user()->isAdmin()||Auth::user()->isEmployee())
                    <td class="sm:p-3 py-2 px-1 border-b border-gray-200 text-right">
                        <div class="flex items-center justify-end">
                            <div class="table_center">
                                <div id="dropdown{{$raport->id}}" class="drop-down">
                            ...
                            <circle cx="5" cy="12" r="1"></circle>
                                    ...
                                    <div class="drop-down__menu-box">
                                        <ul class="drop-down__menu">
                                            <form id="deleteForm" action="{{ route('raporty.destroy', $raport->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <div class="flex items-center">
    ...
    <!-- Main modal -->
    <div id="addRaport"  tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="flex items-start justify-between p-4 border-b rounded-t">
                    ...
                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide="addRaport">
                ...
                <div class="space-y-6">
                    <div class="space-y-6">
                        <form id="addForm" action="{{ route('raporty.store') }}" method="POST">
                            @csrf
                            <div class="p-6">
                                <select name="placowka_id" id="placowka" class="form-control bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
