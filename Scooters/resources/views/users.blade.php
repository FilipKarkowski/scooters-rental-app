...
@section('content')
    ...
    <table class="w-full text-left transition-opacity ease-in-out duration-100">
        ...
        @foreach($clients as $client)
                            ...
                            <div id="dropdown{{$client->id}}" class="drop-down">
                                ...
                                <div class="drop-down__menu-box">
                                    <ul class="drop-down__menu">
                                        ...
                                        <li data-name="role" class="drop-down__item" data-modal-target="roleModal{{$client->id}}" data-modal-toggle="roleModal{{$client->id}}">Rola</li>
                                        <form id="deleteForm{{$client->id}}" action="{{ route('users.destroy', $client->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <div class="flex items-center">
                ...
                <div class="relative w-full max-w-2xl">
                    ...
                    <div class="relative bg-white rounded-lg shadow ">
                        ...
                        <div class="flex items-start justify-between p-4 border-b rounded-t ">
                            <h2 class="text-xl font-bold mb-4">Edytuj Klienta</h2>
                        ...
                        <div class="modal-body p-4">
                            <form action="{{ route('employees.update', $client->id) }}" method="POST">
                                @csrf
                                @method('PUT')

                                <div class="mb-4">
                                    <label for="name" class="block text-gray-800  font-semibold mb-1">Nazwa</label>
                                    <input type="text" class="form-control bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" id="name" name="name" value="{{ $client->name }}" required>
                                </div>

                                <div class="mb-4">
                                    <label for="email" class="block text-gray-800  font-semibold mb-1">Kontakt</label>
                                    <input type="email" class="form-control bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" id="email" name="email" value="{{ $client->email }}" required>
                                </div>
            ...
            <div id="roleModal{{$client->id}}" class="modal fixed inset-0 flex items-center justify-center z-50 hidden">
                ...
                <div class="modal-container bg-white w-full mx-auto rounded shadow-lg z-50 max-w-md">
                    <div class="modal-content py-4 text-left px-6">
                        ...
                        <!-- Modal body -->
                        <div class="my-4">
                            <form action="{{ route('pracownicy.changerole', $client->id) }}" method="POST">
    ...
    <div id="addClientOnline" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <!-- Modal body -->
                <div class="space-y-6">
                    <form id="addClientOnline" action="{{ route('users.storeklient') }}" method="POST">
                        @csrf
                        <div class="p-6">
